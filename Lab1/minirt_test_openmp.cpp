...
int main(int argc, char **argv) {
    ...
    int num_of_threads = (argc > 3 ? std::stoi(argv[3]) : 1);
    int numOfSamples = 1;
...
#pragma omp parallel default(none) shared(viewPlaneResolutionX, viewPlaneResolutionY, viewPlane, numOfSamples, image, scene) num_threads(num_of_threads)
    {
//#pragma omp parallel for default(none) shared(viewPlaneResolutionX, viewPlaneResolutionY, viewPlane, numOfSamples, image, scene) num_threads(num_of_threads)
